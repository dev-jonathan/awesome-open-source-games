...
//           reject(new Error('RATE_LIMIT'));
//         } else if (res.statusCode === 404) {
//           resolve(null); // Repo deleted/moved
//         } else {
//           reject(new Error(`Status ${res.statusCode}: ${data}`));
//         }
//       });
...
async function run() {
  ...
  for (const game of rawGames) {
    ...
    // --- Image Matching ---
    const gameImages = selections[game.id] || [];
    finalGame.hasImage = gameImages.length > 0;

    // Map local dev paths. Cloudflare Pages prefixes handled in Vite plugin/env
    finalGame.images = gameImages.map((file, idx) => {
      const suffix = idx === 0 ? "" : `_${idx}`;
      return `/games/${game.id}/${game.id}${suffix}.webp`;
    });
    ...
    if (!game.link || game.isCompany) {
    ...
    } else if (!game.link.includes("github.com")) {
      issues.nonGithubLinks.push({ id: game.id, link: game.link });
      finalGame.stats = {
        stars: 0,
        commits: 0,
        lastPush: null,
      ...
      issues.rateLimited.push({ id: game.id, link: game.link });
      finalGame.stats = existingStats || {
        stars: 0,
        commits: 0,
        lastPush: null,
      ...
      const match = game.link.match(/github\.com\/([^/]+\/[^/]+)/);
      if (match) {
        ...
        issues.malformedGithubLinks.push({ id: game.id, link: game.link });
        finalGame.stats = {
          stars: 0,
          commits: 0,
          lastPush: null,
